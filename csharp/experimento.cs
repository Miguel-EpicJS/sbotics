...
async Task DirecaoPa(double mult = 1, int dir = 1) {
    ...
    if(dir == 1) {
        while(( Bot.GetComponent<Servomotor>( "fmotor" ).Angle ) < 70) {
            IO.Print("Pa1");
            await Time.Delay(50);
            await FuncPa(mult, dir);
        }
    } else {
        while(( Bot.GetComponent<Servomotor>( "fmotor" ).Angle ) > -10) {
            IO.Print("Pa1");
            await Time.Delay(50);
            await FuncPa(mult, dir);
        }
...
async Task Frente(string[] nomes, double mult = 1, double speed = 0) {
    if(speed == 0) {
        foreach(string nome in nomes) {
        ...
        foreach(string nome in nomes) {
            Bot.GetComponent<Servomotor>(nome).Apply( speed, speed );
        }
    }
...
async Task Stage1() {
    ...
    while(true) {
        ...
        if( ( Bot.GetComponent<ColorSensor>( "lc" ).Analog ).Brightness < 90 && (Bot.GetComponent<ColorSensor>( "rc" ).Analog ).Brightness < 90) {
