...
async Task Main()
{
    ...
    while(true) {
        ...
        IO.Print($"{Math.Round(Bot.GetComponent<UltrasonicSensor>("bultra").Analog)}");
        await Time.Delay(50);
        Up(speed);
    ...
    if (counter == 1)
    {
        while(Bot.GetComponent<UltrasonicSensor>("tultra").Analog == -1)
        {
            await Time.Delay(50);
            Down(speed);
        }
        ...
        while(!VerifyUltra("ffultra", 5))
        {
            await Time.Delay(50);
        }
        await TurnAngle(rspeed, -135);
        Up(speed*2);
    }
    else
    {
